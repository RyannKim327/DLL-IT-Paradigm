...
export default function Calendar(props){
	...
	for(let i = 0; i < days.length; i++){
		if(initialDate.getDay() === i && month === initialDate.getMonth()){
			dates.push(<span className="text-[#efde50 font-bold">{ days_[i] }</span>)
		}else{
	...
	for(let i = 1; i <= bdate.getDate() + day; i++){
		if(i <= day){
			...
			if(initialDate.getMonth() === month && (i - day) === initialDate.getDate() && year === initialDate.getFullYear()){
				dates.push(<span onClick={ () => { checkEvent(month, (i - day), year) }} className="font-bold cursor-pointer border-solid border-[#ffde10] border-2 rounded-[.25rem] text-[#ffde10] p-[0.25rem]">{i - day}</span>)
			}else{
